function d3BarChartDesktop(chartData, reportType, chartTitle, chartDivId, legendDivId, chartAreaWidth, chartAreaHeight, margin) {
	
	var margins = 
		{
			top: margin, 
			bottom: margin, 
			left: margin + (margin * 0.005), 
			right: margin - (margin * 0.05)
		};
	...
	d3.selectAll('.axis')
		.style('font-size', 'calc(' + xScale.bandwidth()/30 + 'vw)')
	...
	barGroups 
		...
		.text((a) => `${a.percent}%`)
		.style('font-size', 'calc(' + xScale.bandwidth()/10 + 'vw)')
	
	// Size of percentages in the middle of the bars
	d3.selectAll('.value')
		.style('font-size', 'calc(' + xScale.bandwidth()/25 + 'vw)') // '14px'
		.style('font-weight', 'bold')
		.each(function (d, i) {
	...
	svg.append('text')
		...
		.attr('y', margins.left / 3.4)  // .attr('y', margin / 2.4)
	...
	svg.append('text')
		...
		.attr('text-anchor', 'middle')
		.text(chartTitle)
		.style('font-size', 'calc(' + chartAreaWidth/100 + 'vw)')  // .style('font-size', (chartAreaWidth / 2 * 0.008) + "em") 
	...
	svg.append('text')
		.attr('class', 'source')
		.attr('x', chartAreaWidth - margins.right / 0.85) //  - margin / 2
		...
		var rows = tbody.selectAll("tr")
			...
			.on('mouseover', function(d, i) {
				...
				d3.selectAll('.divergence')
					.style('font-size', 'calc(' + xScale.bandwidth()/10 + 'vw)') //'14px'
					.style('fill', '#2F4A6D')
					.style('font-weight', 'bold')
					//.each(function (d, i) {
					//	var percent = d3.select(this);
