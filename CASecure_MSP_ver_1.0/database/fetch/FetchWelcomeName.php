	...
	try {
		...
		try {
			...
			$db->beginTransaction();
				...
				$metaXML->addChild('Name', "Fetch Welcome Name");
				$metaXML->addChild('Description', "This query returns the given first name of the current user.");
				...
				$fetchWelcomeNameQuery = $db->prepare($fetchWelcomeNameSQL);
				$fetchWelcomeNameQuery->bindParam(":currentUser", $currentUser, PDO::PARAM_STR);
				$start = microtime(true);
				$fetchWelcomeNameQuery->execute();
				$end = microtime(true);
				$welcomeName = ($fetchWelcomeNameQuery->fetch(PDO::FETCH_ASSOC))['welcome_name'];
				
				$tuple = $resultXML->addChild('user');
				$tuple->addChild('welcome_name', $welcomeName);
				
				$evaluationXML = $queryXML->addChild('Evaluation');
				$evaluationXML->addChild('Time', round((($end - $start) * 1000), 3).'ms');
				$evaluationXML->addChild('Number_of_Results', 1);
				
			$db->commit();
		}
		catch (\PDOException $e) {
			$errorXML = $queryXML->addChild('Error');
			$errorXML->addChild('Code', $fetchWelcomeNameQuery->errorCode());
