	...
	try {
		...
		try {
			...
			$db->beginTransaction();
				...
				$metaXML->addChild('Name', "Fetch Users List");
				$metaXML->addChild('Description', "This query returns the Username and ID of all Users.");
				...
				$fetchUsersListQuery = $db->query($fetchUsersListSQL);
				$end = microtime(true);
				$usersListArray = $fetchUsersListQuery->fetchAll(PDO::FETCH_ASSOC);
				
				$j = 0;
				while (isset($usersListArray[$j])) {//$searchSitesQuery->fetch(PDO::FETCH_ASSOC)) {
					$row = $usersListArray[$j];
				...
				//$queryXML->addChild('SQL', $fetchUsersListSQL);
				
			$db->commit();
		}
		catch (\PDOException $e) {
			$errorXML = $queryXML->addChild('Error');
			//$errorXML->addChild('Code', $fetchUsersListQuery->errorCode());
