	...
	try {
		...
		try {
			...
			$db->beginTransaction();
				...
				$metaXML->addChild('Name', "Fetch Sites With Out Operators List");
				$metaXML->addChild('Description', "This query returns all sites that are not of type 'operator' accessable to the current user.");
				...
				foreach($accessableSites as $key => $accessableSite) {
					...
					$fetchSitesSQL .=
						...
						"FROM sites ".
						"WHERE site_name = ? AND site_type <> 'operator' ";
				}
				$fetchSitesSQL .=
			...
			//$errorXML->addChild('Code', $fetchBigFixUserNameQuery->errorCode());
			//$errorXML->addChild('Code', $fetchAccessableSitesListQuery->errorCode());
			//$errorXML->addChild('Code', $fetchSitesQuery->errorCode());
