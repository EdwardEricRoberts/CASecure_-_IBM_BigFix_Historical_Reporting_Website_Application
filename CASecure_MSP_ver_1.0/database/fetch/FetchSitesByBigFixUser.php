	...
	try {
		...
		try {
			...
			$db->beginTransaction();
				...
				$metaXML->addChild('Name', "Fetch Sites By BigFix User");
				$metaXML->addChild('Description', "This query returns all Sites accessable to a particular BigFix Console User.");
				...
				$fetchAccessableSitesListSQL = 
					"SELECT a.site_name, s.site_display_name, s.site_type ".
					"FROM bigfix_site_access a, sites s ".
					"WHERE a.bigfix_user_name = :bigFixUserName AND s.site_name = a.site_name AND s.site_type <> 'operator' ".
					"ORDER BY s.bigfix_site_id;";
				...
				//print_r($accessableSitesArray);
				//echo "<br/><br/>";
				
				$j = 0;
				while (isset($accessableSitesArray[$j])) {//$searchSitesQuery->fetch(PDO::FETCH_ASSOC)) {
					$row = $accessableSitesArray[$j];
				...
				//$queryXML->addChild('SQL', $fetchAccessableSitesListSQL);
				
			$db->commit();
		}
		catch (\PDOException $e) {
			$errorXML = $queryXML->addChild('Error');
			$errorXML->addChild('Code', $fetchAccessableSitesListQuery->errorCode());
