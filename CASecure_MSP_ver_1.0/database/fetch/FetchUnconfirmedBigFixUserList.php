	...
	try {
		...
		try {
			...
			$db->beginTransaction();
				...
				$metaXML->addChild('Name', "Fetch Unconfirmed BigFix User List");
				$metaXML->addChild('Description', "This query returns all BigFix Console Users whos Passwords have not been confirmed in the portal yet.");
				...
				$fetchBigFixUsersListSQL = 
					"SELECT bigfix_user_name, bigfix_user_id, bigfix_server ".
					"FROM bigfix_logins ".
					"WHERE confirmed = false ".
					"ORDER BY bigfix_user_id;";
				$start = microtime(true);
				$fetchBigFixUsersListQuery = $db->query($fetchBigFixUsersListSQL);
				$end = microtime(true);
				$bigFixUsersListArray = $fetchBigFixUsersListQuery->fetchAll(PDO::FETCH_ASSOC);
				
				$j = 0;
				while (isset($bigFixUsersListArray[$j])) {//$searchSitesQuery->fetch(PDO::FETCH_ASSOC)) {
					$row = $bigFixUsersListArray[$j];
					$tuple = $resultXML->addChild('bigfix_login'); //Tuple
