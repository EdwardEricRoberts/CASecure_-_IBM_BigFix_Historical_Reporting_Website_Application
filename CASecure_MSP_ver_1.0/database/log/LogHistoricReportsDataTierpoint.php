			...
			$db->beginTransaction();
				...
				foreach($siteXML->Query->Result->Tuple as $tuple) {
					...
					if ($patchComplianceDataXML->Query->Result->Tuple->count() != 0) {
						...
						$reportQuery = pdoMultiInsert("historic_patch_compliance_report", $reportDataArray, $db);
						$reportQuery->execute();
				...
				$changeLogMessage = 'Logged Historic Report Data for the Computers of each Site and Computer Group (Patch Compliance Report)';
				
				$changeLogSQL = 
					"INSERT INTO database_change_log ".
					"(timestamp, type_of_change, action_taken, affected_tables ) ".
					"VALUES (:timestamp, 'Historic Report Data Log', :message, 'historic_patch_compliance_report, historic_computer_groups');";
