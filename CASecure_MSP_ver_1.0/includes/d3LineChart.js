	...
	var zoom = d3.zoom()//d3.behavior.zoom()
		...
		.on('zoom', function zoomHandler() {
			...
			backRect.on('mousemove', function() {
				...
				tt.html(tooltipFormatter(_date, _temp, chartData[i].applicable_1, chartData[i].installed_1));
...
*/
	...
	function tooltipFormatter(date, temp, applicable, installed) {
		...
		return dayFormat(date) + ', ' + 
				dateFormat(date) + '<br>' + 
				timeFormat(date) + '<br>' + 
				'Applicable: <b>' + applicable + ' patches</b><br>' +
				'Installed: <b>' + installed + ' patches</b><br>' +
	...
	svg.append('text')
		...
		.attr('y', height + margin.top + margin.bottom * 0.9)
		.attr('text-anchor', 'end')
		.text('CAS Severn, ' + (new Date()).getFullYear())
		.style('font-size', (chartAreaWidth / 2 * 0.0009) + "em")
	...
	svg.append('text')
		...
		.attr('y', height + margin.top + margin.bottom * 0.9) // .attr('y', height + margin * 1.7)
		.attr('text-anchor', 'middle')
		.text('time')
