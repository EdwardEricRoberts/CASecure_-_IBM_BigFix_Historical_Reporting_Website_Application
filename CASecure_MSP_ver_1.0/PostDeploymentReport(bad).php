...
<?php
	...
	function logSummary() {
		$summary = json_decode($_POST[''], true);
	}
...
<script>
	...
	var postDeploymentProxy = "proxies/MicrosoftPatchComplianceReport.php?user=" + curUser + "&pass=" + password + "&serv=" + server + "&cg=All%20Machines";
	...
	function xmlTableParser(proxyURL, tableID) {
		...
		request.onreadystatechange = function() {
			if ((this.readyState === 4) && (this.status === 200)) {
				...
				//alert (compliancePct);
				var queryArray = new Array();//[deploymentArray[0], deploymentArray[1], deploymentArray[2], compliancePct + "%"];
				queryArray[0] = "";
				queryArray[1] = deploymentArray[0];
				queryArray[2] = deploymentArray[1];
				queryArray[3] = deploymentArray[2];
				queryArray[4] = compliancePct + "%";
				/*
				function sendSummaryToDatabase() {
					//var microsoftPatchCompliance
					var database = "database/MicrosoftPatchComplianceSummaryLog.php?appl=" + deploymentArray[0] + "&inst=" + deploymentArray[1] + "&out=" + deploymentArray[2] + "&comp=" + compliancePct + "%25";
				...
				*/
				...
				$(document).ready(function() {
					$("#log").click(function() {
						$.post( 
							$("#sqlStuff").attr("action"),
							$("#str").val(JSON.stringify(queryArray)), 
							function() { 
							}
						);
					});
					
					$("#sqlStuff").submit( function() {
						return false;
					});
				});
				*/
				<?php
					//$sqlInsert = array("<script>document.write(deploymentArray[0])</script>", "<script>document.write(deploymentArray[1])</script>", "<script>document.write(deploymentArray[2])</script>", "<script>document.write(compliancePct)</script>");
					//echo '<script type="text/javascript">alert("$sqlInsert")</script>';
				?>
...
<button onclick="sendSummaryToDatabase()" onmouseover="" style="cursor:pointer;">Log Results</button>
<!--
<form id="sqlStuff" action="<?php echo base_url('admin/logSummary'); ?>" method="post">
	<input type="hidden" id="str" name="str" value="" />
	<input type="submit" id="log" name="queryLog" value="Log Results" />
</form>
-->
...
<div style=" min-width:1000px; max-width:1000px; margin-top: 200px; position: absolute; top: 490" id="mainDiv">
	<table id="postDeploymentTable" class="sortable">
		<th>Computer ID</th><th>Computer</th><th>Users</th><th>Operating System</th><th>IP Address</th><th>Last Report Time</th><th>Applicable Patches</th><th>Installed Patches</th><th>Outstanding Patches</th><th>Compliance</th>
	</table>
