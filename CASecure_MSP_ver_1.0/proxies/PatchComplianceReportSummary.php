<?php
	...
	$siteGroup = str_replace('%20', ' ', $siteGroup);
	$siteGroup = str_replace('%2F', '/', $siteGroup);
	$siteGroup = str_replace(' ', '_', $siteGroup);
	
	//$siteGroup = json_decode($_GET['sg']);
	//print_r($siteGroup);
	echo $siteGroup."<br/>";
	print_r(json_decode($_GET['sg']));
	
	//if (sizeof($siteGroup) == 0)
	//	$siteGroup = array(0);
	/*
...
/*	
	...
	foreach($siteGroup as $siteNum => $site) {
		if ($siteNum != 0) {
			$relevance .= '; ';
		}
		$relevance .= 
