function d3PieChart(data, reportType, chartTitle, chartDivId, legendDivId, chartAreaWidth, chartAreaHeight, margin) {
	...
	var textCategory = 
		arcs.append('svg:text')
			.attr('transform', function(d) { 
				...
				//alert((x/h))
				//if () {
					return 'translate(' + (x/h * (radius + pullOutDistance * 2)) + ',' + (y/h * (radius + pullOutDistance * 2.95)) + ')'; // 30  20
				//}
				//else if () {
			...
			.append("tspan")
				...
				.style('font-size', 'calc(' + ((chartAreaWidth - (radius * 2)) / 2) / 300 + 'vw)')
	...
	svg.append('text')
		...
		.attr('y', margin * 0.75) // 40  margin / 2   chartAreaHeight/ 15
		.attr('text-anchor', 'middle')
		.text(chartTitle)
			.style('font-size', 'calc(' + chartAreaWidth/400 + 'vw)')//(chartAreaWidth / 2 * 0.008) + "em") // '22px'
			.style('font-weight', 600);
	
	svg.append('text')
		.attr('class', 'source')
		.attr('x', chartAreaWidth - margin / 3)//width - 160)
