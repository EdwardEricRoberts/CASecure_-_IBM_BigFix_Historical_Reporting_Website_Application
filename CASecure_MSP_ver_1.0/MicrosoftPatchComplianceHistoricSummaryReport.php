	  ...
	  <div class="span-24">
         <div style="background-color:#cacaca; width:100%; margin-top:2px; height:23px; line-height:23px;"> 
            <span style="text-align:center;">
               ...
               <span style="z-index:0; position:absolute;"><a href="Reporting.php" class="breadcrumblink">Reporting</a> &nbsp;&raquo;&nbsp; Microsoft Patch Compliance Historic Summary</span>
            </span>
         </div>
      </div>
<!-- end breadcrumbs -->
 <br>
      <div class="span-24 last">
	     <div class="pagetitle2" style="margin-bottom:20px; margin-left:10px; float:left">Microsoft Patch Compliance Historic Summary<br>
		 </div>	  
	  </div>
...
<script>
	...
	var historicDataFetchURL = "http://localhost/Server/CASecure_MSP_ver_1.0/database/fetch/FetchHistoricPatchComplianceReportData.php?cid=" + currentUserID + "&cg=" + computerGroup;
		...
		if ((this.readyState === 4) && (this.status === 200)) {
			...
			if (historicDataFetchXML.getElementsByTagName("Error").length == 0) {
				...
				for (var i = 0; i < historicDataFetchCount; i++) {
					...
					historicDataFetchRowHTML += '<td style="display:none">' + historicDataFetchXML.getElementsByTagName("log_id")[i].childNodes[0].nodeValue + '</td>';
					...
					historicDataFetchRowHTML += '<td style="display:none"></td>';
					//historicDataFetchRowHTML += '<td>' + systemCount + '</td>';
					historicDataFetchRowHTML += '</tr>';
	...
	compGroup.onreadystatechange = function() {
		if ((this.readyState === 4) && (this.status === 200)) {
			...
			for (var i = 0; i < rowCount; i++) {
				rowHTML = "<li><a href='MicrosoftPatchComplianceHistoricSummaryReport.php?cg=" + xmlDoc.getElementsByTagName("Answer")[i].childNodes[0].nodeValue + "'>" + xmlDoc.getElementsByTagName("Answer")[i].childNodes[0].nodeValue + "</a></li>";
			   ...
			   <table id="sourceSeverityTable" style="border:1px solid lightgrey; table-layout:fixed; ">
			      <thead>
				     <tr>
						...
						<th style="display:none">Log User</th>
					 </tr>
				  </thead>
